    ...
    <section className="mx-auto grid max-w-[760px] gap-8 lg:grid-cols-[1fr_300px] lg:items-start">
      <div className="space-y-5">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-[11px] text-gray-400">
            Click a date to set your available hours
          </p>
          ...
          {loadingDates && (
            <div className="mt-3 flex items-center justify-center gap-2 text-[11px] text-gray-400">
              <Clock className="size-3.5 animate-spin" /> Loading…
            </div>
          )}

          {/* Legend */}
          <div className="mt-4 flex gap-4 border-t border-gray-100 pt-4 text-[10px] text-gray-500">
          ...
          {!selectedDate ? (
            <div className="py-8 text-center">
              <p className="text-[13px] text-gray-400">
                Select a date on the calendar to set your available hours
              </p>
            </div>
          ) : (
            <>
                ...
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-gray-400">
                    {currentSlots.length === 0
                      ? "No time slots set — add one below"
                      : `${currentSlots.length} time slot${currentSlots.length > 1 ? "s" : ""}`}
                  </p>
                </div>
                <button
                  type="button"
              ...
              {currentSlots.length === 0 ? (
                <div className="mt-4 rounded-lg border-2 border-dashed border-gray-200 py-6 text-center">
                  <p className="text-[12px] text-gray-400">
                    Click "Add Slot" to set working hours for this day
                  </p>
                </div>
              ) : (
              ...
              {/* Status messages */}
              {dateMessage && (
                <p className="mt-3 rounded-md border border-green-200 bg-green-50 px-3 py-2 text-[11px] text-green-700">
                  {dateMessage}
                </p>
              )}
              {dateError && (
          ...
          {!upcomingAppointments || upcomingAppointments.length === 0 ? (
            ...
            upcomingAppointments.slice(0, 5).map((appt, i) => (
              <div
                ...
                className="mb-2 rounded-md bg-blue-50 p-3"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-[10px] text-gray-500">
                      {appt.submitted_on || appt.date || "Scheduled"}
                    </p>
                  </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <dl className="mt-5 space-y-4 text-[12px]">
          ...
          <div>
            <dt className="text-gray-500">Clinic Address</dt>
            <dd className="mt-1 font-medium text-slate-900">
              {doctorInfo.address}
            </dd>
          </div>
